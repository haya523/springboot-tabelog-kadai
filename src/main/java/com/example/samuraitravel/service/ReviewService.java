...
public class ReviewService {
    ...
    public Review create(Integer houseId, Integer userId, ReviewForm form) {
        reviewRepository.findByHouse_IdAndUser_Id(houseId, userId)
                .ifPresent(r -> { throw new IllegalStateException("この店舗には既にレビュー済みです。"); });
    ...
    // ---- 店舗詳細ページ向けユーティリティ（Repositoryは既存メソッドのみ使用） ----
    ...
    public List<Review> findByHouse(House house) {
        ...
        return reviewRepository.findByHouse_Id(house.getId(), pageable).getContent();
    }

    /** 平均評価（Repositoryの平均クエリは使わず、Java側で算出） */
    @Transactional(readOnly = true)
    public Double averageRating(Integer houseId) {
        List<Review> all = reviewRepository.findByHouse_Id(houseId, Pageable.unpaged()).getContent();
